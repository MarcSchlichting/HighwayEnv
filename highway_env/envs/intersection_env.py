...
from highway_env.vehicle.controller import ControlledVehicle2


class IntersectionEnv(AbstractEnv):
    INTRUDER_POSITION_DEVIATION = 10.0
    """ This will be multiplied by a random number between
        -1 and 0.5 to purturb the intruder's initial longitudinal position.
    """
    INTRUDER_SPEED_DEVIATION = 1.0
    """ This will be multiplied by a random number between
        -1 and 1 to purturb the intruder's initial speed.
    """
    EGO_INIT_POS_PERTURBATION = (-15.0, 10.0)
    """ Range of perturbation for ego's initial position. """
    EGO_SPEED_MULTIPLIER_RANGE = (0.7, 1.0)
    """ Ego's initial speed = lane speed limit * random number in this range """
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        # Challenger vehicle
        intruder_vehicle = self._spawn_vehicle(
            75,
            spawn_probability=1,
            go_straight=False,
            position_deviation=self.INTRUDER_POSITION_DEVIATION,    # FOR RANDOMIZATION!!
            speed_deviation=self.INTRUDER_SPEED_DEVIATION,        # FOR RANDOMIZATION!!
            route=np.array([1,2])
        )
        intruder_position = intruder_vehicle.position
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            # )
            destination = "o" + str(self.np_random.integers(1, 4))
            # FOR RANDOMIZATION!!!
            ego_initial_position = ego_lane.position(60 + 5 * 0, 0)
            ego_initial_position[1] += random.uniform(*self.EGO_INIT_POS_PERTURBATION)

            while np.linalg.norm(intruder_position - ego_initial_position) < 20:
                # Don't want the two vehicles to start off too close
                ego_initial_position[1] += (ego_initial_position[1] - 
                                            intruder_position[1]) * 0.1 

            ego_vehicle = ControlledVehicle2(
                self.road,
                ego_initial_position,
                # FOR RANDOMIZATION!!!
                speed=ego_lane.speed_limit * random.uniform(*self.EGO_SPEED_MULTIPLIER_RANGE),   
    ...
    def _spawn_vehicle(
        ...
        vehicle_type = utils.class_from_path(self.config["other_vehicles_type"])
        longitudinal += 5 + random.uniform(-1.0, 0.5) * position_deviation
        speed = 8 + random.uniform(-1.0, 1.0) * speed_deviation
