...
class IntersectionEnv(AbstractEnv):
    ...
    def _make_vehicles(self, n_vehicles: int = 10) -> None:
        ...
        self._spawn_vehicle(
            ...
            spawn_probability=1,
            go_straight=False,
            position_deviation=10.0,    # FOR RANDOMIZATION!!
            speed_deviation=0.2,        # FOR RANDOMIZATION!!
            route=np.array([1,2])
        )
        ...
        for ego_id in range(0, self.config["controlled_vehicles"]):
            ...
            ego_initial_position[1] += random.uniform(-15.0, 10.0)
            ego_vehicle = self.action_type.vehicle_class(
                self.road,
                ego_initial_position,
                # FOR RANDOMIZATION!!!
                speed=ego_lane.speed_limit * random.uniform(0.9, 1.0),   
    ...
    def _spawn_vehicle(
        ...
        vehicle_type = utils.class_from_path(self.config["other_vehicles_type"])
        print("creating intruder vehicle")
        ...
        for v in self.road.vehicles:
            if np.linalg.norm(v.position - vehicle.position) < 15:
                print("WILL NOT ADD THIS VEHICLE!!!")
                return
        vehicle.plan_route_to("o" + str(route[1]))
        # FOR RANDOMIZATION!!!
        vehicle.randomize_behavior()
        print("Adding vehicle to road")
